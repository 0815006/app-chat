...
> **配套文档**：[环境搭建与启动指南](./环境搭建与启动指南.md) | [前端 API 调用参考](./前端API调用参考.md) | [数据库初始化 SQL](./数据库初始化.sql)
...
7. [文件存储 (Storage) 配置](#7-文件存储-storage-配置)
8. [备份与恢复](#8-备份与恢复)
9. [内网生产部署](#9-内网生产部署)
10. [常用 CLI 命令速查](#10-常用-cli-命令速查)
11. [故障排查指南](#11-故障排查指南)
12. [附录：PostgreSQL 常用查询](#12-附录postgresql-常用查询)
...
> **完整建表 SQL** 请参见 [数据库初始化.sql](./数据库初始化.sql)，包含建表、注释、RLS 策略、Realtime 发布。
...
### 11.3 注册失败：`Database error saving new user`

1. `profiles` 表不存在 → 执行[数据库初始化 SQL](./数据库初始化.sql)
...
| [数据库初始化.sql](./数据库初始化.sql) | 数据库迁移脚本 | 表结构变更 |

> **提示**：遇到 Supabase 问题先查阅本文档第 11 节故障排查；环境搭建问题查阅启动指南；前端调用细节查阅 API 参考。
